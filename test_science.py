...
data = {
    ...
    'Species': np.random.choice(['Arabidopsis thaliana', 'Zea mays', 'Oryza sativa', 
                                'Solanum lycopersicum', 'Triticum aestivum'], 50),
    'Genotype': np.random.choice(['Wild Type', 'Mutant A', 'Mutant B', 'Transgenic'], 50),
    'Temperature_C': np.round(np.random.uniform(18, 32, 50), 1),
    'Humidity_%': np.random.randint(40, 95, 50),
    'Light_Intensity': np.random.choice(['Low', 'Medium', 'High'], 50, p=[0.2, 0.5, 0.3]),
...
for species in df['Species'].unique():
    ...
    ax.scatter(species_data['Biomass_g'], species_data['Chlorophyll_ug_cm2'], 
...
axes[1].hist(df['Chlorophyll_ug_cm2'], bins=15, edgecolor='black', alpha=0.7, color='green')
...
# 5.4 Bar chart
fig, ax = plt.subplots(figsize=(10, 6))
mean_biomass = df.groupby('Genotype')['Biomass_g'].mean().sort_values(ascending=False)
bars = ax.bar(mean_biomass.index, mean_biomass.values, alpha=0.7, edgecolor='black')
...
data_by_genotype = [df[df['Genotype'] == g]['Gene_Expression'].values for g in genotypes]
parts = ax.violinplot(data_by_genotype, positions=range(len(genotypes)), 
...
for light in df['Light_Intensity'].unique():
    ...
    mean_by_species = light_data.groupby('Species')['Biomass_g'].mean().sort_values()
    ax.plot(range(len(mean_by_species)), mean_by_species.values, 
            marker='o', label=light, linewidth=2, markersize=8)
